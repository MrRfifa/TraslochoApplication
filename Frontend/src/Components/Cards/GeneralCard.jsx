...
export const GeneralCardSMScreen = ({ isOwner }) => {
  const { t } = useTranslation("register");
  return (
    <div className={`${isOwner?"bg-black":"bg-slate-50"}  hover:bg-[#14213D] transition-all duration-500 hover:text-white w-64 h-40 relative flex flex-col items-center justify-center gap-2 text-center rounded-2xl overflow-hidden` }>
      <div className="z-10 space-y-4 ">
        <span className="text-2xl font-semibold ">
          {isOwner ? t("regOwner") : t("regTransporter")}
        </span>
        {isOwner ? (
